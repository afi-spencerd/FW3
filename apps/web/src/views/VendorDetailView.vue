...
onMounted(async () => {
  if (!props.id) return;
  try {
    apply(await api.getVendor(props.id));
  } catch (err) {
...
async function save(): Promise<void> {
  ...
  try {
    ...
    if (props.id) await api.updateVendor(props.id, payload);
    else await api.createVendor(payload);
    await router.push({ name: "vendors" });
  } catch (err) {
...
<template>
  <div class="container" style="max-width: 860px">
    ...
    <div class="panel">
      <div class="toolbar">
        <h2 style="margin: 0">{{ isEdit ? "Edit vendor" : "New vendor" }}</h2>
        <span class="spacer" />
        <button @click="router.push({ name: 'vendors' })">Back</button>
      </div>
      ...
      <fieldset :disabled="!canManage" style="border: none; padding: 0; margin: 0">
        ...
        <div v-if="canManage" class="toolbar" style="margin-top: 1rem">
          ...
          <button @click="router.push({ name: 'vendors' })">Cancel</button>
        </div>
      </fieldset>
    </div>
  </div>
